...
function duplicate() {
  ...
  for (const c of copies) { DEFS[c.type].translate(c, { x: 0.5, y: -0.5 }); app.store.entities.push(c); }
  app.store.selection = new Set(copies.map((c) => c.id));
  app.store.commit();
}

// clipboard (deep copies of entity data, ids assigned on paste)
let clipboard = [];
function copySel() { const s = app.store.selected(); if (s.length) clipboard = s.map((e) => JSON.parse(JSON.stringify(e))); }
function cutSel() { copySel(); del(); }
function paste() {
  if (!clipboard.length) return;
  const made = clipboard.map((c) => { const e = { ...JSON.parse(JSON.stringify(c)), id: uid() }; DEFS[e.type].translate(e, { x: 0.5, y: -0.5 }); app.store.entities.push(e); return e; });
  app.store.selection = new Set(made.map((e) => e.id));
  clipboard = made.map((e) => JSON.parse(JSON.stringify(e))); // cascade so repeated pastes step
  app.store.commit();
}
function zoomFit() {
...
window.addEventListener('keydown', (e) => {
  ...
  else if (mod && e.key.toLowerCase() === 'x' && !typing) { e.preventDefault(); cutSel(); }
