...
export const DEFS = {
  ...
  pinSupport: def({
    ...
    defaults: (at) => ({ x: at.x, y: at.y, angle: 270, size: 0.7, hatch: true }),
    params: [
  ...
  piston: def({
    ...
    draw: (e, pen) => {
      ...
      // cylinder U (open toward pin)
      pen.poly([
        V.add(A, V.scale(n, outer)), V.add(cylBack, V.scale(n, outer)),
...
function drawTriSupport(e, pen, roller) {
  ...
  if (roller) {
    ...
    pen.circlePx(V.add(V.add(baseC, V.scale(bdir, -bh * 0.5)), off), rr * pen.scale, { stroke: INK, w: 1.4, fill: '#fff' });
    groundC = V.add(baseC, V.scale(down, rr * 2));
  }
  const ga = V.add(groundC, V.scale(bdir, bh * 1.25)), gb = V.add(groundC, V.scale(bdir, -bh * 1.25));
  if (e.hatch) pen.hatch(gb, ga, { side: 1, w: 1.4 });
  else pen.line(gb, ga, { w: 1.4 });
}
