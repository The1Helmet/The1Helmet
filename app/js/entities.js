...
export const DEFS = {
  ...
  sliderSlot: def({
    ...
    defaults: (at) => ({ x: at.x, y: at.y, angle: 0, blockW: 0.7, gap: 0.5, guideLen: 2.4 }),
    params: [
    ...
    draw: (e, pen) => {
      ...
      // two parallel rails forming the slot (pair with a Fixed wall for ground)
      const top = V.add(c, V.scale(nrm, hh)), bot = V.add(c, V.scale(nrm, -hh));
      pen.line(V.sub(top, half), V.add(top, half), { w: 1.6 });
      pen.line(V.sub(bot, half), V.add(bot, half), { w: 1.6 });
